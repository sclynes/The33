package sfx
{
	...
	public class SfxrSynth
	{
		...
		public function loadFromSFXFile(file:ByteArray):void {
			file.position = 0;
			file.endian = "littleEndian";

			var version:int = file.readInt();

			params.waveType = file.readInt();
			params.masterVolume = (version == 102) ? file.readFloat() : 0.5;
			
			params.startFrequency = file.readFloat();
			params.minFrequency = file.readFloat();
			params.slide = file.readFloat();
			params.deltaSlide = (version >= 101) ? file.readFloat() : 0.0;
			
			params.squareDuty = file.readFloat();
			params.dutySweep = file.readFloat();
			
			params.vibratoDepth = file.readFloat();
			params.vibratoSpeed = file.readFloat();
			var unusedVibratoDelay:Number = file.readFloat();
			
			params.attackTime = file.readFloat();
			params.sustainTime = file.readFloat();
			params.decayTime = file.readFloat();
			params.sustainPunch = file.readFloat();
			
			var unusedFilterOn:Boolean = file.readBoolean();
			params.lpFilterResonance = file.readFloat();
			params.lpFilterCutoff = file.readFloat();
			params.lpFilterCutoffSweep = file.readFloat();
			params.hpFilterCutoff = file.readFloat();
			params.hpFilterCutoffSweep = file.readFloat();
			
			params.phaserOffset = file.readFloat();
			params.phaserSweep = file.readFloat();
			
			params.repeatSpeed = file.readFloat();
			
			params.changeSpeed = (version >= 101) ? file.readFloat() : 0.0;
			params.changeAmount = (version >= 101) ? file.readFloat() : 0.0;
